...
export class DB {
  ...
  async init() {
    try {
      let creators = (await this.file.all(
        "SELECT * FROM voiceCreators",
      )) as Creator[];
      ...
      this.file.run(
        "CREATE TABLE voiceCreators (id INTEGER PRIMARY KEY AUTOINCREMENT, guildId TEXT, textChannelId TEXT, voiceChannelId TEXT, categoryId TEXT)",
      );
    }
    try {
      ...
      this.file.run(
        "CREATE TABLE rooms (id INTEGER PRIMARY KEY AUTOINCREMENT, voiceChannelId TEXT, ownerId TEXT, cooldown INTEGER)",
      );
    }
    try {
      let settings = (await this.file.all(
      ...
      this.file.run(
        "CREATE TABLE settings (id INTEGER PRIMARY KEY AUTOINCREMENT, guildId TEXT, userId TEXT, name TEXT, userLimit INTEGER, locked INTEGER, visible INTEGER, leave INTEGER)",
      );
    }
  }
}
...
class CreatorsManager extends Collection<string, Creator> {
  ...
  dbSet(guildId: string, creator: Creator) {
    let creatorDb = this.db.file.all(
      "SELECT * FROM voiceCreators WHERE guildId = ? AND categoryId = ?",
      [creator.guildId, creator.categoryId],
    )[0] as Creator;
    if (!creatorDb) {
      this.db.file.run(
        "INSERT INTO voiceCreators (guildId, textChannelId, voiceChannelId, categoryId) VALUES (?, ?, ?, ?)",
        [
          creator.guildId,
          creator.textChannelId,
          creator.voiceChannelId,
          creator.categoryId,
        ],
      );
    } else
      this.db.file.run(
        "UPDATE voiceCreators SET textChannelId = ?, voiceChannelId = ?, categoryId = ? WHERE guildId = ?",
        [
          creator.textChannelId,
          creator.voiceChannelId,
          creator.categoryId,
  ...
  dbDelete(guildId: string, categoryId: string) {
    ...
    this.db.file.run(
      "DELETE FROM voiceCreators WHERE guildId = ? AND categoryId = ?",
      [guildId, categoryId],
    );
  ...
  async create(guildId: string): Promise<Creator> {
    ...
    for (let i = 0; Object.keys(config.buttons).length > i; i++) {
      if (i % 2 === 0) {
        row1.addComponents(
          new ButtonBuilder()
            .setCustomId(Object.keys(config.buttons)[i])
            .setEmoji(
              this.client.emojisStorage.cache
                .get(Object.keys(config.buttons)[i])!
                .toString(),
            )
    ...
    webhook.send({
      embeds: [new EmbedBuilder().settingRoomEmbed(this.client)],
...
class RoomManager extends Collection<string, Room> {
  ...
  async dbSet(room: Room) {
    ...
    if (!roomDb)
      roomDb = await this.db.file.all(
        "SELECT * FROM rooms WHERE voiceChannelId = ?",
        [room.voiceChannelId],
      )[0];
    if (!roomDb) {
      roomDb = room;
      this.db.file.run(
        "INSERT INTO rooms (voiceChannelId, ownerId, cooldown) VALUES (?, ?, ?)",
        [room.voiceChannelId, room.ownerId, room.cooldown],
      );
    } else {
  ...
  dbDelete(voiceChannelId: string) {
    ...
    this.db.file.run("DELETE FROM rooms WHERE voiceChannelId = ?", [
      voiceChannelId,
    ]);
...
class SettingsManager extends Collection<string, Setting> {
  ...
  async dbGet(userId: string): Promise<Setting | undefined> {
    ...
    if (!result) {
      ...
      this.db.file.run(
        "INSERT INTO settings (userId, name, userLimit, locked, visible) VALUES (?, ?, ?, ?, ?)",
        [
          result.userId,
          result.name,
          result.userLimit,
  ...
  async dbSet(setting: Setting) {
    ...
    if (!settingDb)
      settingDb = await this.db.file.all(
    ...
    if (!settingDb) {
      ...
      if (
        settingDb.name !== setting.name ||
        settingDb.userLimit !== setting.userLimit ||
        settingDb.locked !== setting.locked ||
        settingDb.visible !== setting.visible
      )
        this.db.file.run(
          `UPDATE settings SET 
